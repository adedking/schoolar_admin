import { Form, Modal, Checkbox } from 'carbon-components-react';
import React, { useEffect, useState } from 'react';
import AppButton from '../../../../components/app-button';
import { Loading, Stack } from '@carbon/react';
import moment from 'moment';
import { useGetstudentsBySubClass } from '../../../../redux/classes/hook';
import { useMarkSubjectAttendance } from '../../../../redux/subjects/hook';

const MarkSubjectAttendance = ({isOpen, closeModal, subjectInfo, attendanceInfo=null}) => {
    const [form, setForm] = useState(null)

    const { data: students, isLoading: studentsLoading } = useGetstudentsBySubClass(
        subjectInfo?.sub_class?.id,
        1000,
        1,
    );
    ...
    useEffect(() => {
        if (attendanceInfo) {
            setForm({
                ...
                subject_id: attendanceInfo.subject_id,
                students: attendanceInfo.students ? attendanceInfo.students : []
            })
        } else if (subjectInfo && students) {
            ...
            setForm({
                ...
                main_class_id: subjectInfo.main_class?.id,
                sub_class_id: subjectInfo.sub_class?.id,
                subject_id: subjectInfo.id,
                students: studentData
            })
        }

    }, [students, subjectInfo, attendanceInfo])
    ...
    const {mutateAsync: markAttendance, isLoading: markAttendanceLoading} = useMarkSubjectAttendance()
                ...
                <Stack gap={5}>
                    <div className='flex flex-col gap-2 w-full mt-2'>
                        <div className='flex gap-2 p-2 bg-white'>
                            ...
                            <span className='font-normal'>{subjectInfo?.main_class?.name} - {subjectInfo?.sub_class?.name}</span>
                        </div>
                        <div className='flex gap-2 p-2 bg-white'>
                            <span className='font-semibold'>Subject:</span>
                            <span className='font-normal'>{subjectInfo?.name}</span>
                        </div>
